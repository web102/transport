...
public class ConvertUtil {
    ...
    public static String hexstr2bin(String hex) {
        ...
        if (hex != null) {
            ...
            for (int i = 0; i < hex.length() - 1; i++) {
                result = result.concat(hex2bin(hex.substring(i, i + 1)));
            }
     ...
     * @return 长度为8的二进制字符串
     */
    public static String int2bin8(int value) {
    ...
    public static String bytes2hex(byte[] b) {
        ...
        if (b != null && b.length > 0)
            for (int i = 0; i < b.length; i++) {
                value = (byte) ((b[i] >>> 4) & 0x0F);
                result += Integer.toHexString(value);
                value = (byte) (b[i] & 0x0F);
                result = result.concat(Integer.toHexString(value)).concat(" ");
            }

        return result.toUpperCase();
    }

    public static String bytes2Numhex(byte[] b) {
        String result = "";
        byte value;
    ...
    public static String bytes2hex(byte[] b, int pos, int length) {
        String result = "";
        byte value;
        ...
        if (src != null && insertStr != null) {
            ...
            if (index < 0) {
                if (index * -1 > src.length())
                    ...
                    result = src.substring(0, src.length() + index + 1)
                            + insertStr + temp;
                }
    ...
    public static String interChange(String src) {
        ...
        if (src != null) {
            ...
            for (int i = 0; i < src.length() - 2; i += 2) {
                result = result.concat(src.substring(i + 1, i + 2));
                result = result.concat(src.substring(i, i + 1));
            }
        }
     ...
     * @return
     */
    public static String bytes2hexstr(byte[] b, char seprator) {
     ...
     * 把字节数组转化成十六进制的字符串
     */
    public static String bytes2hexstr(byte[] b, int position, int len) {
        String result = null;

        if (b != null && b.length > 0) {
            for (int i = 0; i < len; i++) {
                String hex = byte2hex(b[position + i]);
                if (result == null) {
                    result = hex;
                } else {
                    result += hex;
    ...
    public static String bytes2str(byte[] b) {
        ...
        if (b != null && b.length > 0) {
            ...
            byte[] b2 = bytesCopy(b, 0, actualLen);
        ...
        if (src != null) {
            ...
            for (int i = 0; i < ss.length; i++) {
                if (ss[i] != null && ss[i].length() > 0) {
                    result = result.concat(ss[i]).concat(" ");
                }
            }
        ...
        if (src != null && src.length() == src.getBytes().length) {
            ...
            for (int i = 0, j = 0; i < b.length; i++) {
                ...
                if (j == 0)
                    ...
                    left = (byte) (b[i] >> j);
                    hex = byte2hex((byte) value);
                    result += hex;
                    if (i == b.length - 1)
                        result += byte2hex(left);
                }
            }
    ...
    public static boolean isMobileNo(String msg) {
        ...
        if (msg != null && msg.length() == 11) {
            if (isNumeric(msg)
                    && (msg.substring(0, 2).equals("13") || msg.substring(0, 2)
                    .equals("15"))) {
                return true;
            }
        }
     ...
     * @return
     */
    public static ArrayList<String> sortByLen(ArrayList<String> list,
                                              boolean long2short) {
    ...
    public static byte[] reverseBytes(byte[] srcBytes) {

        for (int i = 0; i < srcBytes.length / 2; i++) {
            byte temp = srcBytes[i];
            srcBytes[i] = srcBytes[srcBytes.length - 1 - i];
            srcBytes[srcBytes.length - 1 - i] = temp;
        }
        return srcBytes;
     ...
     *
     * @param placename 地名类型 0:省 1:市 2:区 3:县
     * @return
     */
    ...
    public static boolean isUnicommMobile(String sim) {
        ...
        if (sim != null && sim.length() == 11) {
            ...
            if (part.equals("130") || part.equals("131") || part.equals("132")
                    || part.equals("133") || part.equals("153")
                    || part.equals("156"))
                result = true;
        }
        return result;
    ...
    public static boolean isChinaMobile(String sim) {
        ...
        if (sim != null && sim.length() == 11) {
            String part = sim.substring(0, 3);
            if (part.equals("134") || part.equals("135") || part.equals("136")
                    || part.equals("137") || part.equals("138")
                    || part.equals("139") || part.equals("159")
                    || part.equals("158"))
                result = true;
        }
        return result;
    ...
    public static String CmdToString(int[] cmdInt, int len) {
        int loop = 0;
        String cmdStr = "";
        for (loop = 0; loop < len; loop++) {
            cmdStr = cmdStr + (char) cmdInt[loop];
        }
        return cmdStr;
    }
        ...
        if (str != null && pos != null) {
            for (int i = 0; i < str.length(); i++) {
                ...
                if (index + pos.length() == str.length()
                        || (index != -1 && str.substring(index + pos.length())
                        .indexOf(" ") == 0)) {
                    result = index;
                    break;
                }
        ...
        if (isTelNo(telno) && telno.length() > 7) {
            ...
            switch (len) {
                ...
                case 11:
                    ...
                    break;
                case 12:
                    if (telno.indexOf("098") == 0 || telno.indexOf("094") == 0
                            && telno.indexOf("0943") == -1
                            || telno.indexOf("092") == 0
                            || telno.indexOf("086") == 0
                            || telno.indexOf("084") == 0
                            || telno.indexOf("0827") == 0
                            || telno.indexOf("0829") == 0
                            || telno.indexOf("0822") == 0
                            || telno.indexOf("0824") == 0
                            || telno.indexOf("080") == 0
                            || telno.indexOf("07437") == 0
                            || telno.indexOf("0483") == 0
                            || telno.indexOf("0788") == 0)
        ...
        if (msg != null) {
            ...
            for (int i = 0; i < atoms.length; i++) {
                ...
                if (bs.length == 1) {
                    ...
                    if (getUnsigned(bs[0]) < 0x81 || getUnsigned(bs[0]) > 0xFE
                            || getUnsigned(bs[1]) < 40
                            || getUnsigned(bs[1]) > 0xFE)
                        return true;
                }
    ...
    public static ArrayList<String> readTxtFile2(String fileName)
            throws IOException {
    ...
    public static String formatIP(String ip) {
        ...
        if (ip != null) {
            ...
            if (p != null && p.length == 4) {
                result = getFixedLenStr(p[0], 3, '0');
                result += getFixedLenStr(p[1], 3, '0');
                result += getFixedLenStr(p[2], 3, '0');
                result += getFixedLenStr(p[3], 3, '0');
            }
        }
        return result;
    }
     ...
     *
     * @param className 类名
     * @return Class文件所在的路径，不包括Class文件本身的包路径。比如：com.gftech.web.Test,返回的路径格式如下/E:/
    ...
    public static String getWebinfPath(String classPath) {
        ...
        if (classPath != null) {
            ...
            for (int i = 1; i < strs.length - 1; i++) {
                ...
                if (strs[i] != null)
                    path = path.concat(strs[i]).concat(
    ...
    public static String dateAToStr(Date date) {
        if (date != null) {
    ...
    public static String dateBToStr(Date date) {
        if (date != null) {
    ...
    public static int getInt(byte byt) {
        return (int) (byt < 0 ? 256 + byt : byt);
    }
    ...
    public static boolean IsUnsign(String value) {
        return Pattern.matches("^\\d*[.]?\\d*$", value);
    }

    public static int transBinayToBcd(int m_byte) {
        return ((m_byte / 10) * 16 + m_byte % 10);
    }

    public static int transBcdToBinay(byte m_byte) {
        int temp = getInt(m_byte);
     ...
     * 转换为时间信息 a (分至年)
     **/
    public final static byte[] toTimeInfoA(Date date) {
        /**
         * D7 D6 D5 D4 D3 D2 D1 D0 IV TIS 分钟 SU RSE RSE 小时 周次 日 PTI ETI 月 RES 年
         */
        ...
        byte[] dateinfo = new byte[5];
        dateinfo[4] = (byte) ((byte) (year - 2000) & 0x7f);
        dateinfo[3] = (byte) ((byte) month & 0x0f);
        dateinfo[2] = (byte) (((byte) dayofweek & 0x07 << 5) | ((byte) day & 0x01f));
        dateinfo[1] = (byte) ((byte) hour & 0x1f);
        dateinfo[0] = (byte) ((byte) minute & 0x3f);
        return dateinfo;
    }
     ...
     * 转换为时间信息 a (分至年)
     **/
    public final static byte[] toTimeInfoBCD5(Date date) {
        /**
         * YH YL MM DD HH FF
         */
        Calendar calendar = Calendar.getInstance();
        ...
        dateinfo[4] = (byte) (ConvertUtil.transBinayToBcd(hour));
        dateinfo[3] = (byte) (ConvertUtil.transBinayToBcd(day));
        dateinfo[2] = (byte) (ConvertUtil.transBinayToBcd(month));
        dateinfo[1] = (byte) (ConvertUtil.transBinayToBcd(year % 100));
        dateinfo[0] = (byte) (ConvertUtil.transBinayToBcd(year / 100));
        return dateinfo;
    }
     ...
     * 转换为时间信息 a (分至年)
     **/
    public final static byte[] toTimeInfoBCD(Date date) {
        /**
         * YH YL MM DD HH FF
         */
        Calendar calendar = Calendar.getInstance();
        ...
        byte[] dateinfo = new byte[6];
        dateinfo[5] = (byte) (ConvertUtil.transBinayToBcd(minute));
        dateinfo[4] = (byte) (ConvertUtil.transBinayToBcd(hour));
        dateinfo[3] = (byte) (ConvertUtil.transBinayToBcd(day));
        dateinfo[2] = (byte) (ConvertUtil.transBinayToBcd(month));
        dateinfo[1] = (byte) (ConvertUtil.transBinayToBcd(year % 100));
     ...
     * 时间转换为BCD byte数组 (秒至年)
     **/
    public final static byte[] toTimeInfoBCD7(Date date) {
        /**
         * YH YL MM DD HH FF
         */
        Calendar calendar = Calendar.getInstance();
        ...
        byte[] dateinfo = new byte[7];
        dateinfo[6] = (byte) (ConvertUtil.transBinayToBcd(second));
        dateinfo[5] = (byte) (ConvertUtil.transBinayToBcd(minute));
        dateinfo[4] = (byte) (ConvertUtil.transBinayToBcd(hour));
        dateinfo[3] = (byte) (ConvertUtil.transBinayToBcd(day));
        dateinfo[2] = (byte) (ConvertUtil.transBinayToBcd(month));
     ...
     * 转换为时间信息 b (毫秒至年)
     **/
    public final static byte[] toTimeInfoB(Date date) {
        /**
         * D7 D6 D5 D4 D3 D2 D1 D0 毫秒 秒 毫秒 IV TIS 分钟 SU RSE RSE 小时 周次 日 PTI ETI
         * 月 RES 年
         */
        Calendar calendar = Calendar.getInstance();
        int year = calendar.get(Calendar.YEAR);
        ...
        byte[] dateinfo = new byte[7];
        dateinfo[6] = (byte) ((byte) (year - 2000) & 0x7f);
        dateinfo[5] = (byte) ((byte) month & 0x0f);
        dateinfo[4] = (byte) (((byte) dayofweek & 0x07 << 5) | ((byte) day & 0x01f));
        dateinfo[3] = (byte) ((byte) hour & 0x1f);
        dateinfo[2] = (byte) ((byte) minute & 0x3f);
        dateinfo[1] = (byte) (((byte) (millsecond >> 8 & 0x0003)) | ((byte) second & 0x3f << 2));
        dateinfo[0] = (byte) (millsecond & 0x00ff);

        return dateinfo;

    }

    public static Date dateinfoAtoDate(byte[] dateinfo) {
        if (dateinfo.length == 5) {
            Calendar calendar = Calendar.getInstance();
            calendar.set(Calendar.MINUTE, dateinfo[0] & 0x3f);
            calendar.set(Calendar.HOUR_OF_DAY, dateinfo[1] & 0x1f);
            calendar.set(Calendar.DAY_OF_MONTH, dateinfo[2] & 0x1f);
            calendar.set(Calendar.MONTH, (dateinfo[3] & 0x0f) - 1);
            calendar.set(Calendar.YEAR, dateinfo[4] + 2000);
    ...
    public static Date dateinfoBCDtoDate(byte[] dateinfo) {
        if (dateinfo.length == 6) {
            Calendar calendar = Calendar.getInstance();
            calendar.set(Calendar.MINUTE, transBcdToBinay(dateinfo[5]));
            calendar.set(Calendar.HOUR_OF_DAY, transBcdToBinay(dateinfo[4]));
            calendar.set(Calendar.DAY_OF_MONTH, transBcdToBinay(dateinfo[3]));
            calendar.set(Calendar.MONTH, transBcdToBinay(dateinfo[2]) - 1);
            calendar.set(Calendar.YEAR, transBcdToBinay(dateinfo[1]) * 100
                    + transBcdToBinay(dateinfo[0]));
            return calendar.getTime();
        } else {
            return null;
        }
    ...
    public static Date dateinfoBCDtoDate5(byte[] dateinfo) {
        if (dateinfo.length == 5) {
            Calendar calendar = Calendar.getInstance();
    ...
    /**
     * 四川规约使用 格式为：dd mm yy hh ff ss
     **/
    public static Date bytestoDateSC(byte[] dateinfo) {
        if (dateinfo.length == 6) {
            Calendar calendar = Calendar.getInstance();
            calendar.set(Calendar.SECOND, dateinfo[5]);
            calendar.set(Calendar.MINUTE, dateinfo[4]);
            calendar.set(Calendar.HOUR_OF_DAY, dateinfo[3]);
            calendar.set(Calendar.DAY_OF_MONTH, dateinfo[0]);
            calendar.set(Calendar.MONTH, dateinfo[1] - 1);
            calendar.set(Calendar.YEAR, 2000 + dateinfo[2]);
            return calendar.getTime();
        } else {
            return null;
        }
    ...
    public static Date dateinfoBCD7toDate(byte[] dateinfo) {
        if (dateinfo.length == 7) {
            Calendar calendar = Calendar.getInstance();
            calendar.set(Calendar.SECOND, transBcdToBinay(dateinfo[6]));
            calendar.set(Calendar.MINUTE, transBcdToBinay(dateinfo[5]));
            calendar.set(Calendar.HOUR_OF_DAY, transBcdToBinay(dateinfo[4]));
            calendar.set(Calendar.DAY_OF_MONTH, transBcdToBinay(dateinfo[3]));
            calendar.set(Calendar.MONTH, transBcdToBinay(dateinfo[2]) - 1);
    ...
    public static Date dateinfoBtoDate(byte[] dateinfo) {
        if (dateinfo.length == 7) {
            Calendar calendar = Calendar.getInstance();

            calendar.set(Calendar.MILLISECOND, byte2int(dateinfo[0]) + 256
                    * (dateinfo[1] & 0x03));
            calendar.set(Calendar.SECOND, (dateinfo[1] >> 2) & 0x3f);
            calendar.set(Calendar.MINUTE, dateinfo[2] & 0x3f);
            calendar.set(Calendar.HOUR_OF_DAY, dateinfo[3] & 0x1f);
            calendar.set(Calendar.DAY_OF_MONTH, dateinfo[4] & 0x1f);
            calendar.set(Calendar.MONTH, (dateinfo[5] & 0x0f) - 1);
            calendar.set(Calendar.YEAR, 2000 + dateinfo[6]);
    ...
    public static Date bcdDateinfoToDate(byte[] dateinfo) {
        if (dateinfo.length == 6) {
            Calendar calendar = Calendar.getInstance();
            calendar.set(Calendar.SECOND, transBcdToBinay(dateinfo[5]));
            calendar.set(Calendar.MINUTE, transBcdToBinay(dateinfo[4]));
            calendar.set(Calendar.HOUR_OF_DAY, transBcdToBinay(dateinfo[3]));
            calendar.set(Calendar.DAY_OF_MONTH, transBcdToBinay(dateinfo[2]));
            calendar.set(Calendar.MONTH, (transBcdToBinay(dateinfo[1]) - 1));
            calendar.set(Calendar.YEAR, 2000 + transBcdToBinay(dateinfo[0]));
    ...
    public static String GetStringInParentheses(String cmdString, int pos) {
        String value = "";
        pos = cmdString.indexOf('(', pos) + 1;
        if (pos == 0) {
            return "";
        }
        do {
            value = value + cmdString.charAt(pos++);
        } while (cmdString.charAt(pos) != ')');
        return value;
    }
     ...
     * 时间转换为BCD byte数组 (秒至年)
     **/
    public final static byte[] toTimeInfoBCD6(Date date) {
        /**
         * YH YL MM DD HH FF
         */
        Calendar calendar = Calendar.getInstance();
        ...
        int second = calendar.get(Calendar.SECOND);
        byte[] dateinfo = new byte[6];
        dateinfo[5] = (byte) (ConvertUtil.transBinayToBcd(second));
        dateinfo[4] = (byte) (ConvertUtil.transBinayToBcd(minute));
        dateinfo[3] = (byte) (ConvertUtil.transBinayToBcd(hour));
        dateinfo[2] = (byte) (ConvertUtil.transBinayToBcd(day));
        dateinfo[1] = (byte) (ConvertUtil.transBinayToBcd(month));
        dateinfo[0] = (byte) (ConvertUtil.transBinayToBcd(year % 100));
        return dateinfo;
    }
     ...
     * @return
     */
    public static float byte2Float(byte byte1, byte byte2, byte byte3,
                                   byte byte4) {
        int tempInt = fourBytesToInt(byte1, byte2, byte3, byte4);
        return intBitsToFloat(tempInt);
    }

    public static float byte2Float(byte[] byte1) {
        int tempInt = fourBytesToInt(byte1[0], byte1[1], byte1[2], byte1[3]);
        return intBitsToFloat(tempInt);
    }

    // 是否低位在前
    public static float byte2Float(byte[] byte1, boolean islittend) {
        int tempInt;
        if (islittend) {
            tempInt = fourBytesToInt(byte1[3], byte1[2], byte1[1], byte1[0]);
        } else {
            tempInt = fourBytesToInt(byte1[0], byte1[1], byte1[2], byte1[3]);
        }
    ...
    public static int fourBytesToInt(byte b1, byte b2, byte b3, byte b4) {
        return (getInt(b1) << 24) + (getInt(b2) << 16) + (getInt(b3) << 8)
                + getInt(b4);
    }

    public static float intBitsToFloat(int bits) {
        /* s 为符号（sign）；e 为指数（exponent）；m 为有效位数（mantissa） */
        int s = (bits >> 31) == 0 ? 1 : -1, e = (bits >> 23) & 0xff, m = (e == 0) ? (bits & 0x7fffff) << 1
                : (bits & 0x7fffff) | 0x800000;
        return s * m * (float) Math.pow(2, e - 150);
    }

    // 是否低位在前
    public static double byte2Double(byte[] byte1, boolean isLitlEnd) {
        long tempInt = ConvertUtil.bytes2long(byte1, isLitlEnd);
        return Double.longBitsToDouble(tempInt);
    }

    public static int transBcdToBinay(int m_byte) {
        if (((byte) (m_byte) & (byte) (0x80)) == (byte) 0x80) {
            return ((((byte) (m_byte) & (byte) (0x7f)) >> 4) * 10
                    + ((byte) (m_byte) & (byte) (0x7f)) % 16 + 8 * 10);
        } else {
            return ((m_byte >> 4) * 10 + m_byte % 16);
        }
    }

    /**
     * @param m_byte
     * @return DLT645 2007规约用
     */
    public static int transBcd2ToBinay(byte[] m_byte) {
        int result = 0;
        result += transBcdToBinay(m_byte[1]) * 100;
        result += transBcdToBinay(m_byte[0]) * 1;
        return result;
    }

    public static int transBcd2ToBinay(byte[] m_byte, boolean ishightfirst) {
        int result = 0;
        if (ishightfirst) {
            result += transBcdToBinay(m_byte[0]) * 100;
            result += transBcdToBinay(m_byte[1]) * 1;
        } else {
            result += transBcdToBinay(m_byte[1]) * 100;
            result += transBcdToBinay(m_byte[0]) * 1;
        }
     ...
     * @return DLT645 2007规约用
     */
    public static int transBcd3ToBinay(byte[] m_byte) {
        int result = 0;
        result += transBcdToBinay(m_byte[2]) * 10000;
        result += transBcdToBinay(m_byte[1]) * 100;
        result += transBcdToBinay(m_byte[0]) * 1;
        return result;
    }
     ...
     * @return DLT645 2007规约用
     */
    public static int transBcd4ToBinay(byte[] m_byte) {
        int result = 0;
        result += transBcdToBinay(m_byte[3]) * 1000000;
        result += transBcdToBinay(m_byte[2]) * 10000;
        result += transBcdToBinay(m_byte[1]) * 100;
        result += transBcdToBinay(m_byte[0]) * 1;
        return result;
    ...
    public static double transBcd8_4ToBinay(byte[] m_byte) {
        double result = 0;
        result += transBinayToBcd(m_byte[0]) * 1000000;
        result += transBinayToBcd(m_byte[1]) * 10000;
        result += transBinayToBcd(m_byte[2]) * 100;
        result += transBinayToBcd(m_byte[3]) * 1;
        result += transBinayToBcd(m_byte[4]) * 0.01;
        result += transBinayToBcd(m_byte[5]) * 0.0001;
        return result;
    }

    public static double transBcd8_4ToBinay1(byte[] m_byte) {
        double result = 0;
        result += transBcdToBinay(m_byte[0]) * 1000000;
        result += transBcdToBinay(m_byte[1]) * 10000;
        result += transBcdToBinay(m_byte[2]) * 100;
        result += transBcdToBinay(m_byte[3]) * 1;
        result += transBcdToBinay(m_byte[4]) * 0.01;
        result += transBcdToBinay(m_byte[5]) * 0.0001;
        return result;
    }

    public static double transBcd6_2ToBinay(byte[] m_byte) {
        double result = 0;

        result += transBinayToBcd(m_byte[0]) * 10000;
        result += transBinayToBcd(m_byte[1]) * 100;
        result += transBinayToBcd(m_byte[2]) * 1;
        result += transBinayToBcd(m_byte[3]) * 0.01;

        return result;
    }

    public static double transBinayToBcd(byte[] m_byte, double factor) {
        double result = 0;
        for (int i = 0; i < m_byte.length; i++) {
            result = result + transBcdToBinay(m_byte[i])
                    * Math.pow(10, 2 * (m_byte.length - i - 1));
        }
        result = result * factor;
        return result;
    }

    public static long transBinayToBcd(byte[] m_byte) {
        long result = 0;
        for (int i = 0; i < m_byte.length; i++) {
            result = result
                    + (long) (transBcdToBinay(m_byte[i]) * Math.pow(10,
                    2 * (m_byte.length - i - 1)));
        }
    ...
    public static byte[] str4ToByte(String tempStr) {
        if (tempStr != null && tempStr.length() >= 4) {

            byte[] code = new byte[2];
            String str[] = {"00", "00"};
            if (tempStr.length() >= 2) {
                str[1] = tempStr.substring(tempStr.length() - 2,
                        tempStr.length());

            }
            if (tempStr.length() >= 4) {
                str[0] = tempStr.substring(tempStr.length() - 4,
                        tempStr.length() - 2);
            }
            code[0] = (byte) str2ToByte(str[0]);
            code[1] = (byte) str2ToByte(str[1]);
    ...
    public static Byte str2ToByte(String tempStr) {
        byte temp = 0;
        if (tempStr != null && tempStr.length() == 2) {
            ...
            for (int i = 0; i < tempStr.length(); i++) {
                ...
                if (tempchar >= '0' && tempchar <= '9') {
                    temp = (byte) (temp + (tempchar - '0') << (4 * (tempStr
                            .length() - 1 - i)));

                } else if (tempchar >= 'A' && tempchar <= 'F') {
                    temp = (byte) (temp + (tempchar - 55) << (4 * (tempStr
                            .length() - 1 - i)));
                }
     ...
     *
     * @param b
     * @return Bits Description 63 Reserved (0) 31 - 62 UTC (seconds since Jan
     * 1, 1970 12AM GMT) 0 - 30 Fractional seconds (units = 1/2147
     * microseconds)
     */
    public static Date bytes2Date(byte[] b) {
        ...
        if (b.length == 8) {
            ...
            long secondstemp = ConvertUtil.bytes2long(ConvertUtil.bytesCopy(b,
                    0, 4));
            long seconds = ((secondstemp & 0x7FFFFFFF) << 1)
                    + (long) ((b[4] & 0x80) >> 7);

            long millseconds = (long) ((ConvertUtil.bytes2long(ConvertUtil
                    .bytesCopy(b, 4, 4)) & 0x7FFFFFFF)) / 2147000l;

            return new Date(seconds * 1000l + millseconds);
            // return new Date(seconds*1000l+28800000l+millseconds);
        }
        return null;
    }
    ...
    public static byte[] date2byte(Date date) {
        ...
        if (date != null) {
            ...
            // int seconds=(int)(millseconds/1000)-28800;
            int seconds = (int) (millseconds / 1000);
            int mill = (int) (millseconds % 1000);

            int mircroseconds = mill * 2147000;

            temp[7] = (byte) (mircroseconds & 0x000000ff);
            temp[6] = (byte) ((mircroseconds & 0x0000ff00) >> 8);
            temp[5] = (byte) ((mircroseconds & 0x00ff0000) >> 16);
            temp[4] = (byte) (((mircroseconds & 0x7f000000) >> 24) + ((seconds & 0x0000001) << 7));
            temp[3] = (byte) ((seconds & 0x000001fe) >> 1);
            temp[2] = (byte) ((seconds & 0x0001fe00) >> 9);
            temp[1] = (byte) ((seconds & 0x01fe0000) >> 17);
            temp[0] = (byte) ((seconds & 0xfe000000) >> 25);
            return temp;
        }
        return null;
    }
    ...
    /**
     * Get heximal bytes format from a time value in long.
     * <p>
     * output
     * input
     *
     * @return valid length of buff[]
     */
    public static byte[] dlmsDate2DayBytes(Date date) {
        byte[] buff = new byte[12];
        Calendar cld = Calendar.getInstance();
        cld.setTime(date);
        int pos = 0;
        buff[pos++] = (byte) (cld.get(Calendar.YEAR) >> 8);
        buff[pos++] = (byte) (cld.get(Calendar.YEAR) & 0xff); // year
        buff[pos++] = (byte) (cld.get(Calendar.MONTH) + 1); // month
        buff[pos++] = (byte) (cld.get(Calendar.DAY_OF_MONTH)); // day
        /*
         * buff[pos++] = (byte) (0x01); // day of week, not specified
		 * buff[pos++] = (byte) (cld.get(Calendar.HOUR_OF_DAY)); // hour
		 * buff[pos++] = (byte) (cld.get(Calendar.MINUTE)); // minute
		 * buff[pos++] = (byte) (cld.get(Calendar.SECOND));// seconds
		 */
        buff[pos++] = (byte) (0xff); // hundredths
        buff[pos++] = (byte) (0xff);
        buff[pos++] = (byte) (0xff); // deviation of time zone
        buff[pos++] = (byte) (0xff); // clock status
        buff[pos++] = (byte) (0xff); // hundredths
        buff[pos++] = (byte) (0xff);
        buff[pos++] = (byte) (0xff); // deviation of time zone
        buff[pos++] = (byte) (0xff); // clock status
    ...
    public static byte[] dlmsDate2YearBytes(Date date) {
        byte[] buff = new byte[12];
        Calendar cld = Calendar.getInstance();
        cld.setTime(date);
        int pos = 0;
    ...
    public static byte[] dlmsDate2Bytes(Date date) {
        byte[] buff = new byte[12];
        Calendar cld = Calendar.getInstance();
        cld.setTime(date);
        int pos = 0;
        ...
        buff[pos++] = (byte) (cld.get(Calendar.MONTH) + 1); // month
        buff[pos++] = (byte) (cld.get(Calendar.DAY_OF_MONTH)); // day
        buff[pos++] = (byte) (cld.get(Calendar.DAY_OF_WEEK)); // day of week,
        // not specified
        buff[pos++] = (byte) (cld.get(Calendar.HOUR_OF_DAY)); // hour
        buff[pos++] = (byte) (cld.get(Calendar.MINUTE)); // minute
        buff[pos++] = (byte) (cld.get(Calendar.SECOND));// seconds
    ...
    public static Date dlmsBytes2Date(byte[] buffer) {
        Calendar cld = Calendar.getInstance();
        cld.set(Calendar.YEAR, (getInt(buffer[0]) * 256 + getInt(buffer[1])));
        cld.set(Calendar.MONTH, buffer[2] - 1);
        cld.set(Calendar.DAY_OF_MONTH, buffer[3]);
        cld.set(Calendar.HOUR_OF_DAY, buffer[5]);
        cld.set(Calendar.MINUTE, buffer[6]);
        cld.set(Calendar.SECOND, buffer[7]);
     ...
     * @return
     */
    public static int getBit1InByte(byte[] b) {
        int nos = 0;
        if (b != null) {
            for (int i = 0; i < b.length; i++) {
                if (b[i] == 0) {
    ...
    public static HashMap<Integer, Boolean> getBit1InByte(byte[] b,
                                                          boolean isHighFirst) {
        ...
        if (b != null) {
            if (isHighFirst) {
                for (int i = 0; i < b.length; i++) {
                    if (b[i] == 0) {
                        for (int j = 0; j < 8; j++) {
                            hashMap.put(nos++, false);
                        }
                    } else {
                        for (int j = 0; j < 8; j++) {
                            if (((b[i] >> j) & 0x01) == 0x01) {
                                hashMap.put(nos++, true);
                            } else {
                                hashMap.put(nos++, false);
                            }
                        }
    ...
    public static String bytesTo8bin(byte[] bytes){
        String bin = "";
        for(int i = 0 ; i<bytes.length ;i++){
            byte b = bytes[i];
            String shortBin = Integer.toBinaryString(b&0xFF);
            switch (8-shortBin.length()){
                case 1:shortBin="0"+shortBin;break;
                case 2:shortBin="00"+shortBin;break;
                case 3:shortBin="000"+shortBin;break;
                case 4:shortBin="0000"+shortBin;break;
                case 5:shortBin="00000"+shortBin;break;
                case 6:shortBin="000000"+shortBin;break;
                case 7:shortBin="0000000"+shortBin;break;
                case 8:shortBin="00000000"+shortBin;break;
            }
            bin = bin+shortBin;
        }
        return bin;
    ...
    public static void main(String[] args) {
        byte[] a = new byte[]{0xa,0x65};
        System.out.println(ConvertUtil.bytesTo8bin(a));
